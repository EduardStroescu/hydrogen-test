...
import {Params, useNavigate, useRouteLoaderData} from '@remix-run/react';
import {useFrame} from '@react-three/fiber';
import {EllipseCurve, type Group, Vector3} from 'three';
import {
  Float,
  Html,
  Line,
...
const CollectionProducts = memo(function CollectionProducts({
  params,
  width,
}: {
  ...
  const collectionItemsData = useRouteLoaderData<typeof loader>(
  ...
  const points = useMemo(
    () =>
      new EllipseCurve(0, 0, 12, 12, 0, 2 * Math.PI, true, 0).getPoints(300),
    [],
  );

  const tOffset = 0.5; // Offset to stagger the products
  ...
  function handleProductsNavigate(handle: RouteHandle) {
    navigate(
      params.locale
